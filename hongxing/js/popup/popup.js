(function () {
    ...
    n = ["underscore", "angular", "lang", "core/services/tele", "core/services/corruptDetector", "popup/module", "popup/controllers/ConflictController", "popup/controllers/MenuController", "angular_translate"], e = function (n, e, o) {
        return e.module("popup").config(function ($compileProvider, $routeProvider, $translateProvider) {
            return $compileProvider.aHrefSanitizationWhitelist(/^\s*(https?|ftp|mailto|chrome):/), $compileProvider.imgSrcSanitizationWhitelist(/^\s*(https?|data|ftp|mailto|chrome):/), $routeProvider.when("/menu", {templateUrl: "/partials/popup/menu.html", controller: "MenuController", resolve: {popupMenuData: function (tele) {
                return tele.run("popupMenuData")
            }}}).when("/conflict", {templateUrl: "/partials/popup/conflicts.html", controller: "ConflictController"}).otherwise({redirectTo: "/menu"}), o.config($translateProvider)
