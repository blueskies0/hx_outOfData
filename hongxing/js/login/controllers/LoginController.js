(function () {
    ...
    e = ["angular", "login/module", "core/module", "core/services/tele", "core/services/pageUtils", "core/services/validate", "core/services/storage", "login/module"], n = function (e) {
        ...
        return n = function ($scope, $rootScope, $location, $http, $timeout, tele, pageUtils, validate, storage, SERVER, LOGIN_EVENT_NAME, ERROR_LOGIN_UNKNOWN, ERROR_LOGIN_TIME) {
            return $scope.name = $location.search().name || storage.get("lastLoginName", ""), $scope.password = "", $scope.resetPasswordUrl = "https://" + SERVER + "/user/password/reset", $scope.focuses = {name: !$scope.name, password: !!$scope.name}, $scope.submitting = !1, $scope.checkingName = !1, $scope.clearFormValidity = function () {
                ...
                return $scope.login.name.$setValidity("server", !0), $scope.login.name.$stateVisible = !0, $scope.login.password.$stateVisible = !0, $scope.login.$valid ? ($scope.submitting = !0, $http.get("https://" + SERVER + "/user/login", {params: {name: $scope.name, password: $scope.password}}).success(function (e) {
