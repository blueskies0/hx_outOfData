(function () {
    ...
    s = ["angular", "options/module"], o = function (s) {
        ...
        return o = function ($scope, $rootScope, $http, SERVER, $translate) {
            return $scope.oldPassword = "", $scope.newPassword = "", $scope.rePassword = "", $scope.focuses = {oldPassword: !0, newPassword: !1}, $scope.alert = $scope.$parent.alert, $scope.closeModal = $scope.$parent.closeModal, $scope.submitChangePassword = function () {
                return $scope.oldPassword ? $scope.newPassword ? $scope.newPassword !== $scope.rePassword ? ($scope.alert($translate.instant("options.change_password.alert.password2_not_same")), $scope.focuses.newPassword = !0, !1) : ($scope.disableInput = !0, $http({method: "POST", url: "https://" + SERVER + "/user/password", params: {old: $scope.oldPassword.trim(), "new": $scope.newPassword.trim(), sid: $rootScope.user.profile.sid}, headers: {"Content-Type": "application/x-www-form-urlencoded"}}).success(function (s) {
